...
if($_GET['action'] == "table_data"){
      ...
      $datacount = $querycount->fetch_array();
        ...
        {            
         $query = $mysqli->query("Provinsi,Destinasi FROM tbl_kontak order by $order $dir
                                                      LIMIT $limit
                                                      OFFSET $start");
        }
        else {
            $search = $_POST['search']['value']; 
            $query = $mysqli->query("Provinsi,Destinasi FROM tbl_kontak WHERE nama LIKE '%$search%'
        ...
        if(!empty($query))
        {
            ...
            {
                $nestedData['Provinsi'] = $palembang;
                $nestedData['Destinasi'] = $r['lampung'];
